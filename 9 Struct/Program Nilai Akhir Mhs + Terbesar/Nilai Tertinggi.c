...
void nilai_tertinggi(data *tinggi) {
    maks max;
    max.nilai = 0;
    puts("Nilai tertinggi");
    for(int i = 0; i < jumlah; i++) {
        if(tinggi->akhir > max.nilai) {
            max.nilai = tinggi->akhir;
            strcpy(max.nama, tinggi->nama);
            tinggi++;
        }
    }
    printf("Nama        : %s\n", max.nama);
